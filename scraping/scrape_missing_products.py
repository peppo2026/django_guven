...
OUTPUT_FILE = Path("scraping/missing_scraped_products.json")
FAILED_FILE = Path("scraping/failed_missing_products.json")
...
def parse_product_detail(html, source_url):
    ...
    # Sağ/Sol/Yön seçenekleri
    option_title_tag = soup.find(id="MainContent_Lbl_Secenek_turu")

    if option_title_tag:
        data["option_title"] = clean_option_title(option_title_tag.get_text(" "))
    ...
    option_values = clean_option_values(option_values)
...
print("Eksik ürün linki:", len(missing_links))
print("=" * 80)

for index, item in enumerate(missing_links, start=1):
    print("\n" + "=" * 80)
    print(f"{index}/{len(missing_links)} eksik ürün çekiliyor")
    ...
    try:
        ...
        if not product_data["price"]:
            raise Exception("Fiyat boş geldi")
    ...
    print("Başarılı toplam:", len(results))
    print("Hatalı toplam:", len(failed))
